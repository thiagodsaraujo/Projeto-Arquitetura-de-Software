...
//	@PostMapping("/{id}/sacar1/{valor}")
//	public ResponseEntity<Conta> sacar(@PathVariable Long id, @PathVariable Double valor) throws Exception{
//		Conta contaSaque = contaRepository.findById(id)
//				.orElseThrow(()-> new Exception("Conta not found for this id :" + id));
//		contaSaque.setSaldo(contaSaque.getSaldo() - valor);
//		
//		final Conta updateConta = contaRepository.save(contaSaque);
//		
//		return ResponseEntity.ok(contaSaque);
//	}
//	
//	@PostMapping("/{id}/depositar1/{valor}")
//	public ResponseEntity<Extract> depositar(@PathVariable Long id, @PathVariable Double valor) throws Exception{
//		Conta contaDeposito = contaRepository.findById(id)
//				.orElseThrow(()-> new Exception("Conta not found for this id :" + id));
//		Double saldoAnterior = contaDeposito.getSaldo();
//		contaDeposito.setSaldo(contaDeposito.getSaldo() + valor);
...
//		data.getCalendarType();
//		var tipo = TipoOperacao.DEPOSITO;
//		Double saldoAtual = updateConta.getSaldo();
//		
//			Extract extractTest = new Extract(data, tipo , saldoAnterior, saldoAtual, valor, updateConta);
//			System.out.println(extractTest);
//			Extract extractDebit = extractService.generate(true, updateConta, tipo,saldoAnterior,saldoAtual, valor);
//		
//		return ResponseEntity.ok(extractDebit);
//	}
